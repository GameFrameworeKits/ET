...
namespace EGF
{
    ...
    internal sealed partial class ResourceManager : IResourceManager
    {
        ...
        public async UniTask<InitializationOperation> InitPackage(string packageName)
        {
#if UNITY_EDITOR
            //编辑器模式使用。
            EPlayMode ePlayMode = (EPlayMode)UnityEditor.EditorPrefs.GetInt("EditorPlayMode");
            Log.Warning($"Editor Module Used :{ePlayMode}");
#elif UNITY_WEBGL
            EPlayMode ePlayMode = EPlayMode.WebPlayMode;
#else
            //运行时使用。
            ...
            // 单机运行模式
            if (ePlayMode == EPlayMode.OfflinePlayMode)
            {
        ...
        public AssetInfo GetAssetInfo(string location, string packageName = "")
        {
            ...
            else
            {
                ...
                if (package == null)
                {
                    Debug.LogError($"The package does not exist. Package Name :{packageName}");
                }
        ...
        public async void LoadAssetAsync(string location, Type assetType, int priority, LoadAssetCallbacks loadAssetCallbacks, object userData, string packageName = "")
        {
            ...
            {
                string errorMessage = string.Format("Can not load asset '{0}' because :'{1}'.", location, assetInfo.Error);
