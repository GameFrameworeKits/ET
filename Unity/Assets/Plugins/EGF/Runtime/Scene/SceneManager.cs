...
namespace EGF
{
    ...
    internal sealed class SceneManager : ISceneManager
    {
        ...
        public void UnloadScene(string sceneAssetName, object userData, string packageName = "")
        {
            ...
            {
                Debug.LogError("You must set resource manager first.");
            }

            if (SceneIsUnloading(sceneAssetName))
            {
                Debug.LogError(string.Format("Scene asset '{0}' is being unloaded.", sceneAssetName));
            }

            if (SceneIsLoading(sceneAssetName))
            {
                Debug.LogError(string.Format("Scene asset '{0}' is being loaded.", sceneAssetName));
            }

            if (!SceneIsLoaded(sceneAssetName))
            {
                Debug.LogError(string.Format("Scene asset '{0}' is not loaded yet.", sceneAssetName));
            }
        ...
        {
            m_LoadingSceneAssetNames.Remove(sceneAssetName);
            string appendErrorMessage = string.Format("Load scene failure, scene asset name '{0}', status '{1}', error message '{2}'.", sceneAssetName, status, errorMessage);
        ...
        private void UnloadSceneFailureCallback(string sceneAssetName, object userData)
        {
            ...
            Debug.LogError(string.Format("Unload scene failure, scene asset name '{0}'.", sceneAssetName));
        }
            ...
            bool suspendLoad = false, int priority = 100, bool gcCollect = false, string packageName = "", object userData = null)
        {
            ...
            if (SceneIsLoaded(sceneAssetName))
            {
                Debug.LogError(string.Format("Scene asset '{0}' is already loaded.", sceneAssetName));
            }
            ...
            else if (sceneMode == LoadSceneMode.Single)
            {
                ...
                // TODO TEST 建议抛出通知解耦
                //Resource.ForceUnloadUnusedAssets(gcCollect);
            }
